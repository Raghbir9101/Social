  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        // Track the visit with location data — reuse the standard visitor
        // tracking so QNA visits appear in the admin Visitors page. The box is
        // identified by a synthetic campaignId of the form `qna-<boxId>`.
        const visitorInfo = getVisitorInfo();
        const analytics = collectAnalytics();
        api.post('/visitors/track', {
          ...visitorInfo,
          campaignId: `qna-${boxId}`,
          location,
          ...analytics,
        }).catch(() => {
          // Silent fail — don't block UX if tracking fails
        });
      } catch {
        setState('notfound');
      }
